...
object SolutionContext {
  ...
  def apply[A, B](islandId: Int, fitness: A => B, evolver: Evolver[A], mutationRate: Double, population: Seq[A])(
      implicit ordering: Ordering[B]): SolutionContext[A, B] = {
...
/**
  ...
  * @tparam B The type of fitness score
  */
case class SolutionContext[A, B](islandId: Int,
                                 fitness: A => B,
                                 evolver: Evolver[A],
                                 mutationRate: Double,
                                 population: Seq[A],
                                 generations: Int)(implicit val ordering: Ordering[B]) {
  def increment(newPopulation: Seq[A]): SolutionContext[A, B] =
    copy(population = newPopulation, generations = generations + 1)
}
